...
Answer questions about their background, skills, experience, and what roles they are looking for.
Use ONLY the public profile JSON provided — never invent employers, dates, or skills.
...
If the visitor wants to hire, interview, or connect, invite them to use the chat to ask more
or sign up on Hireloop as a recruiter at app.hireloop.in/signup?role=recruiter.
    ...
    *,
    ...
    if settings.openrouter_api_key:
        try:
            ...
            candidate_reply = (response.content or "").strip()
            if candidate_reply:
                reply = candidate_reply
        except Exception as exc:
            logger.warning("public_profile_chat_llm_failed", error=str(exc)[:200])
