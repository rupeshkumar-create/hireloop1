...
GUPSHUP_SETTINGS = {
    ...
    "gupshup_otp_template_id": "otp-template-uuid",
}
...
def make_settings(**overrides: object) -> Settings:
    values = {
        ...
        # Non-default secrets so the production secret guard is satisfied when
        # a test constructs Settings(environment="production").
        "secret_key": "test-secret-key",
        "service_secret": "test-service-secret",
        "gupshup_api_key": "",
        "gupshup_whatsapp_number": "",
...
def test_development_uses_gupshup_when_configured() -> None:
    settings = make_settings(environment="development", **GUPSHUP_SETTINGS)

    assert _select_otp_provider(settings) == "gupshup"
...
def test_production_prefers_gupshup_over_twilio() -> None:
...
def test_production_uses_twilio_when_gupshup_missing() -> None:
    settings = make_settings(
        environment="production",
        twilio_account_sid="AC123",
        twilio_auth_token=FAKE_TWILIO_TOKEN,
...
async def test_development_falls_back_to_local_otp_when_twilio_trial_blocks_number(
    ...
    class _FakeDB:
        """OTP store backing — no prior send (cooldown clear); records writes."""

        def __init__(self) -> None:
            self.executed = 0

        async def fetchrow(self, query: str, *args: object) -> None:
            return None  # no existing row → cooldown passes, no active code
    ...
    assert db.executed >= 1  # the issued OTP was persisted to the shared store
