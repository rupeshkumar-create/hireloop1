...
def test_tool_status_label_is_specific_for_job_search() -> None:
    assert tool_status_label("job_search", voice_mode=False) == "Searching India roles (~8s)…"


def test_tool_status_label_is_spoken_for_voice() -> None:
    assert tool_status_label("job_search", voice_mode=True) == "I'm searching India roles now (~8s)…"
